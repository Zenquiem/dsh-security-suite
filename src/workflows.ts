...
1. Select the narrowest scan type that matches the request: repository/path for standard, Git range for diff, or deep for a multi-pass review. Use \`security_run_deep_investigation\` when the request requires the complete DSH-native deep discovery, validation, attack-path, and finalization workflow; use its resume tool after interruption. Call a scan tool first when it adds evidence.
