...
export async function applyRemediationProposal(workspace: string, config: Config, scanId: string, remediationId: string, approved: boolean): Promise<RemediationProposal> {
  ...
  await persistInvestigationArtifacts(getStateDir(config.stateDir), verification); await saveScan(getStateDir(config.stateDir), verification); proposal.verificationScanId = verification.id; proposal.verification = remediationVerification(verification, finding)
  proposal.state = 'verified'
  proposal.version = (proposal.version ?? 2) + 1
  await saveProposal(getStateDir(config.stateDir), proposal)
  return proposal
}
