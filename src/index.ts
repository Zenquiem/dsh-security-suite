...
import { cancelInvestigation, claimAuditTask, completeScan, pendingCandidates, recordAttackPath, recordTriageDecision, recordValidation, resumeInvestigation } from './workbench.js'
...
export function apply(ctx: Context, config: PluginConfig): void {
  ...
  ctx.tools.register(defineTool({
    name: 'security_record_triage_decision', description: 'Record one append-only triage decision for a finding (codex-security finding_decisions semantics). closed requires a close reason: already_fixed, wont_fix, or false_positive; a false_positive close requires a suppressed or not_applicable validation receipt, and an already_fixed close requires a resolved annotation or reportable validation. The (status, closeReason, note) triple is appended to the audit log only when it changes.', parameters: { scan_id: { type: 'string', required: true, description: 'Saved scan identifier.' }, finding_id: { type: 'string', required: true, description: 'Finding identifier.' }, status: { type: 'string', required: true, enum: ['open', 'closed'], description: 'New triage status.' }, close_reason: { type: 'string', enum: ['already_fixed', 'wont_fix', 'false_positive'], description: 'Required when status is closed.' }, note: { type: 'string', description: 'Optional triage note.' } },
    output: { schema: { type: 'object', properties: { updated: { type: 'boolean' }, findingId: { type: 'string' }, decisions: { type: 'number' } }, required: ['updated', 'findingId', 'decisions'], additionalProperties: false }, render: (_args, value) => [{ type: 'text', text: JSON.stringify(value) }] },
    async execute(args) { return recordTriageDecision(config, args.scan_id, args.finding_id, { status: args.status as 'open' | 'closed', closeReason: args.close_reason as 'already_fixed' | 'wont_fix' | 'false_positive' | undefined, note: args.note }) },
  }))
  ...
  ctx.tools.register(defineTool({
    name: 'security_compare_scans', description: 'Compare two saved scans by stable finding fingerprint and classify findings as new, persisting, reopened, resolved, or unknown when coverage is incomplete. A reopened finding was previously closed (resolved or false positive) and is open again in the later scan.', parameters: { before_scan_id: { type: 'string', required: true, description: 'Earlier scan identifier.' }, after_scan_id: { type: 'string', required: true, description: 'Later scan identifier.' } },
    output: { schema: { type: 'object', properties: { new: { type: 'array', items: { type: 'string' } }, persisting: { type: 'array', items: { type: 'string' } }, reopened: { type: 'array', items: { type: 'string' } }, resolved: { type: 'array', items: { type: 'string' } }, unknown: { type: 'array', items: { type: 'string' } } }, required: ['new', 'persisting', 'reopened', 'resolved', 'unknown'], additionalProperties: false }, render: (_args, value) => [{ type: 'text', text: JSON.stringify(value) }] },
    async execute(args) { const stateDir = getStateDir(config.stateDir); const [before, after] = await Promise.all([loadScan(stateDir, args.before_scan_id), loadScan(stateDir, args.after_scan_id)]); const previous = new Map(before.findings.map(finding => [finding.fingerprint, finding])); const current = new Map(after.findings.map(finding => [finding.fingerprint, finding])); const closedBefore = (finding: { status: string }): boolean => finding.status === 'resolved' || finding.status === 'false_positive'; const result = { new: [] as string[], persisting: [] as string[], reopened: [] as string[], resolved: [] as string[], unknown: [] as string[] }; for (const finding of current.values()) { const prior = previous.get(finding.fingerprint); if (!prior) { result.new.push(finding.id); continue } if (closedBefore(prior) && finding.status === 'open') { result.reopened.push(finding.id); continue } result.persisting.push(finding.id) } for (const finding of previous.values()) { if (!current.has(finding.fingerprint)) (after.coverage.complete ? result.resolved : result.unknown).push(finding.id) } return result },
  }))
