...
export function apply(ctx: Context, config: PluginConfig): void {
  ...
  ctx.tools.register(defineTool({
    name: 'security_deep_get_worklist', description: 'Read the immutable authoritative source worklist for one claimed DSH delegated deep-discovery worker. Items are ordered by frozen local attack-surface signals, which are review leads rather than vulnerability proof. This tool is available only to active workers.', parameters: { job_id: { type: 'string', required: true }, worker_id: { type: 'string', required: true }, claim_token: { type: 'string', required: true } },
    output: { schema: { type: 'object', properties: { digest: { type: 'string' }, items: { type: 'array', items: { type: 'object', properties: { path: { type: 'string' }, sha256: { type: 'string' }, language: { type: 'string' }, priority: { type: 'number' }, riskSignals: { type: 'array', items: { type: 'string' } } }, required: ['path', 'sha256', 'language', 'priority', 'riskSignals'], additionalProperties: false } } }, required: ['digest', 'items'], additionalProperties: false }, render: (_args, value) => [{ type: 'text', text: JSON.stringify(value) }] },
