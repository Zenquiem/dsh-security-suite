...
import { applyRemediationProposal, bulkScan, installPreCommitHook, planCandidateValidation, proposeReviewedRemediation, remediationPlan, rerunSavedScan, resumeBulkJob, rollbackRemediationProposal, runCandidateValidation, runCandidateValidationPlan, runIsolatedValidation, startBulkCsvJob } from './operations.js'
...
import { createDisclosureCampaign, getDisclosureAssignment, readDisclosureSource, runDisclosureCampaign, submitDisclosureReport } from './disclosure.js'
...
export function apply(ctx: Context, config: PluginConfig): void {
  ...
  ctx.tools.register(defineTool({
    name: 'security_start_disclosure_campaign', description: 'Freeze supplied vulnerability notes and source receipts, then run exactly one restricted DSH writer per distinct vulnerability to produce a source-cited disclosure report. This does not modify source, call external services, or create an executable PoC. Set experiment_authorized true only when controlled local experiments are explicitly in scope.', parameters: { source_root: { type: 'string', description: 'Optional workspace-relative source root; defaults to the active workspace.' }, source_revision: { type: 'string', description: 'Optional exact vulnerable revision or release identifier.' }, experiment_authorized: { type: 'boolean', description: 'Whether controlled local reproduction experiments are explicitly authorized; default false.' }, vulnerabilities: { type: 'array', required: true, items: { type: 'object', properties: { id: { type: 'string', required: true }, title: { type: 'string', required: true }, notes: { type: 'string', required: true }, source_paths: { type: 'array', required: true, items: { type: 'string' } } }, additionalProperties: false }, description: 'One to 25 distinct vulnerability inputs. Each receives one isolated writer.' } },
    ...
    async execute(args, exec) { const campaign = await createDisclosureCampaign(process.cwd(), config, args.source_root, args.vulnerabilities.map(item => ({ id: item.id, title: item.title, notes: item.notes, sourcePaths: item.source_paths })), args.experiment_authorized ?? false, args.source_revision); return runDisclosureCampaign(ctx, config, campaign.id, exec.signal) },
  }))
  ...
  ctx.tools.register(defineTool({
    name: 'security_propose_reviewed_remediation', description: 'Create a review-required, bounded source replacement for one structured reportable finding. The proposal does not modify source. It binds an exact source range, original text, replacement text, rationale, test plan, and frozen scan snapshot; the normal approval-gated apply flow rejects any drift.', parameters: { scan_id: { type: 'string', required: true }, finding_id: { type: 'string', required: true }, file: { type: 'string', required: true, description: 'The reportable finding source file.' }, start_line: { type: 'number', required: true }, end_line: { type: 'number', required: true }, expected_text: { type: 'string', required: true, description: 'Exact current text across the inclusive source range.' }, replacement_text: { type: 'string', required: true, description: 'Reviewed replacement text. This is never applied by this tool.' }, rationale: { type: 'string', required: true, description: 'How the replacement closes the validated root cause and preserves expected behavior.' }, test_plan: { type: 'string', required: true, description: 'Focused regression checks to run after application.' } },
    output: { schema: { type: 'object', properties: { id: { type: 'string' }, findingId: { type: 'string' }, file: { type: 'string' }, line: { type: 'number' }, patch: { type: 'string' }, baseSnapshotDigest: { type: 'string' }, status: { type: 'string' }, requiresApproval: { type: 'boolean' }, requiresReview: { type: 'boolean' }, safeToApply: { type: 'boolean' }, rationale: { type: 'string' } }, required: ['id', 'findingId', 'file', 'line', 'patch', 'baseSnapshotDigest', 'status', 'requiresApproval', 'requiresReview', 'safeToApply', 'rationale'], additionalProperties: false }, render: (_args, value) => [{ type: 'text', text: value.patch ?? '' }] },
    async execute(args) { return proposeReviewedRemediation(process.cwd(), config, args.scan_id, args.finding_id, { file: args.file, startLine: args.start_line, endLine: args.end_line, expectedText: args.expected_text, replacementText: args.replacement_text, rationale: args.rationale, testPlan: args.test_plan }) },
  }))
