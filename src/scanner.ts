...
import type { Confidence, DiffScanMode, Evidence, FileReceipt, Finding, Preflight, RuleReceipt, ScanActivity, ScanRecord, Severity, TargetSnapshot } from './contracts.js'
import { assignFindingIdentities, candidateId, createScanId, findingId, getStateDir, scanArtifactDir, sealScan, sha256 } from './state.js'
...
/** Require a local PHP cURL handle to be configured and then executed. */
function phpTlsConfigurationCandidates(file: string, content: string, lines: string[]): Candidate[] {
  const candidates: Candidate[] = []
  for (const match of content.matchAll(/\bcurl_setopt\s*\(\s*(\$[A-Za-z_]\w*)\s*,\s*CURLOPT_SSL_VERIFYPEER\s*,\s*(?:false|0)\s*\)/gi)) {
    ...
    const initialized = new RegExp(`${quotedHandle}\\s*=\\s*curl_init\\s*\\(`).exec(window.slice(0, localOffset))
    const executed = new RegExp(`\\bcurl_exec\\s*\\(\\s*${quotedHandle}\\s*\\)`, 'i').exec(window.slice(localOffset + match[0].length))
    ...
    const candidate = configurationCandidate(CONFIGURATION_RULES.tls, file, lines, controlLine, 'Configuration analysis found CURLOPT_SSL_VERIFYPEER disabled on a local cURL handle that is executed later in the same block.')
    candidate.evidence.push({ kind: 'context', detail: 'The same initialized cURL handle is passed to curl_exec() after certificate verification was disabled.', location: { file, line: sinkLine, excerpt: sourceLine(lines, sinkLine), role: 'sink' } })
    candidates.push(candidate)
  }
  return candidates
}
...
function candidateToFinding(candidate: Candidate): Finding {
  const fingerprint = hash(`${candidate.rule}:${candidate.excerpt.replace(/\s+/g, ' ')}`)
  ...
  const anchor = slug(`${candidate.rule}-${candidate.excerpt}`)
...
function reduceCandidates(candidates: Candidate[]): Finding[] {
  ...
  for (const candidate of candidates) {
    ...
    const existing = byLocation.get(mergeKey)
    ...
    else byLocation.set(mergeKey, finding)
  }
  return [...byLocation.values()].sort((a, b) => a.locations[0].file.localeCompare(b.locations[0].file) || a.locations[0].line - b.locations[0].line)
}
...
export async function runScan(directory: string, limits: ScanLimits, mode: 'standard' | 'deep', threatModel: string, scopeRequested = false, stateDirectory = '', _legacyAutomaticValidation?: false): Promise<ScanRecord> {
...
export async function reviewGitDiff(workspace: string, base: string | undefined, requestedMode: DiffScanMode | 'auto' = 'auto'): Promise<{ mode: string; diff: string; truncated: boolean; baseRevision?: string; headRevision?: string }> {
  ...
  if (requestedMode === 'commit' && base === undefined) throw new Error('Commit diff mode requires a commit ref in base.')
  if (requestedMode === 'branch_diff' && base === undefined) throw new Error('Branch diff mode requires a merge-base or branch ref in base.')
  if (requestedMode === 'working_tree' && base !== undefined) throw new Error('Working-tree diff mode does not accept a base ref.')
  ...
  const args = ['diff', '--no-ext-diff', '--unified=20']; const headRevision = await execFileAsync('git', ['rev-parse', '--verify', 'HEAD'], { cwd: resolve(workspace), encoding: 'utf8' }).then(result => result.stdout.trim()).catch(() => undefined)
  if (requestedMode === 'commit') {
    const commit = await execFileAsync('git', ['rev-parse', '--verify', base!], { cwd: resolve(workspace), encoding: 'utf8' }).then(result => result.stdout.trim())
    const parent = await execFileAsync('git', ['rev-parse', '--verify', `${commit}^`], { cwd: resolve(workspace), encoding: 'utf8' }).then(result => result.stdout.trim()).catch(() => undefined)
    const result = parent
      ? await boundedGitDiff(workspace, [...args, parent, commit])
      : await boundedGitDiff(workspace, ['diff-tree', '--root', '--no-commit-id', '--no-ext-diff', '--unified=20', '-p', commit])
    return { mode: 'commit', diff: result.output, truncated: result.truncated, baseRevision: parent, headRevision: commit }
  }
  const mode = base === undefined ? 'working-tree' : 'branch-diff'
  const baseRevision = base === undefined ? headRevision : await execFileAsync('git', ['rev-parse', '--verify', base], { cwd: resolve(workspace), encoding: 'utf8' }).then(result => result.stdout.trim())
  ...
  if (base !== undefined || truncated) return { mode, diff, truncated, baseRevision, headRevision }
  ...
  for (const path of listed.split('\0').filter(Boolean)) {
    if (!TEXT_EXTENSIONS.has(extname(path).toLowerCase()) && !isGitHubWorkflow(path)) continue
    const remaining = MAX_DIFF_BYTES - Buffer.byteLength(diff, 'utf8'); if (remaining <= 0) return { mode, diff, truncated: true, baseRevision, headRevision }
    const addition = await boundedGitDiff(workspace, ['diff', '--no-index', '--no-ext-diff', '--unified=20', '--', '/dev/null', path], true)
    if (Buffer.byteLength(addition.output, 'utf8') > remaining) { diff += addition.output.slice(0, remaining); return { mode, diff, truncated: true, baseRevision, headRevision } }
    diff += addition.output; truncated ||= addition.truncated
    if (truncated) return { mode, diff, truncated, baseRevision, headRevision }
  }
  return { mode, diff, truncated, baseRevision, headRevision }
}

export async function runDiffScan(workspace: string, base: string | undefined, threatModel: string, stateDirectory = '', _legacyAutomaticValidation?: false, requestedMode: DiffScanMode | 'auto' = 'auto'): Promise<ScanRecord> {
  const effectiveMode: DiffScanMode = requestedMode === 'auto' ? base ? 'branch_diff' : 'working_tree' : requestedMode
  const review = await reviewGitDiff(workspace, base, effectiveMode); const candidates: Candidate[] = []; const receipts: FileReceipt[] = []; const ruleReceipts: RuleReceipt[] = []; const changedPaths = new Set<string>(); const addedLineMap = new Map<string, Set<number>>(); const addedSourceMap = new Map<string, Array<{ line: number; source: string }>>(); let currentFile = ''; let oldLine = 0; let newLine = 0; let addedLines: Array<{ line: number; source: string }> = []
