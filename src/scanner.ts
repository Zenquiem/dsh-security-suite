...
/**
 * Run the existing local ES-module analyzer against current source, then retain
 * only results whose sink or propagation evidence is a newly added diff line.
 * This proves a new call path without restating unchanged historical findings.
 */
async function semanticDiffCandidates(root: string, addedLines: Map<string, Set<number>>): Promise<{ candidates: Candidate[]; parseErrors: number }> {
  const inventory = await collectFiles(root, { maxFiles: 10_000, maxFileBytes: 1_048_576 })
  const modules = await Promise.all(inventory.files.filter(file => ['.js', '.jsx', '.mjs', '.cjs', '.ts', '.tsx'].includes(extname(file))).map(async file => ({ file: relative(root, file), source: await readFile(file, 'utf8') })))
  const analysis = analyzeJavaScriptModuleGraph(modules)
  return { candidates: analysis.candidates.filter(candidate => candidateTouchesAddedLine(candidate, addedLines)), parseErrors: analysis.parseErrors.length }
}
...
export async function runDiffScan(workspace: string, base: string | undefined, threatModel: string, stateDirectory = ''): Promise<ScanRecord> {
  ...
  for (const diffLine of review.diff.split(/\r?\n/)) {
    ...
    if (diffLine.startsWith('+') && !diffLine.startsWith('+++')) {
      ...
      if (currentFile !== 'unknown') { const lines = addedLineMap.get(currentFile) ?? new Set<number>(); lines.add(newLine); addedLineMap.set(currentFile, lines) }
      newLine++; continue
    }
    if (diffLine.startsWith('-') && !diffLine.startsWith('---')) {
      if (currentFile !== 'unknown') changedPaths.add(currentFile)
