...
import { createDisclosureCampaign, getDisclosureAssignment, loadDisclosureCampaign, readDisclosureSource, runDisclosureCampaign, submitDisclosureReport } from '../src/disclosure.ts'
...
test('disclosure campaign assigns one restricted DSH writer per vulnerability and writes standalone reports', async () => {
  ...
  try {
    ...
    assert.equal(result.lifecycle, 'completed'); assert.equal(fake.count(), 2); assert.deepEqual(fake.restrictions[0], ['security_disclosure_get_assignment', 'security_disclosure_read_source', 'security_disclosure_submit_report'])
